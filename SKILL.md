...
license: MIT
tags: [skill-generation, github, automation, learning, sharing]
...
- **Smart Categorization**: Organizes repos into 12 domains (Bitcoin/Lightning, AI/ML, Privacy, Finance, etc.)
...
- **Shareable**: Generated skills follow Hermes conventions and can be installed by any Hermes agent
...
graph LR
    ...
    D --> E[Sharing / Distribution]
    E --> F[Continuous Learning via Cron]
```

## 🤝 Shareability

Skills generated by StarLearner-Nexus follow standard Hermes SKILL.md conventions, so they can be installed by any Hermes agent and shared like any other skill (for example, via a git repository or by exchanging the SKILL.md file directly). Each generated skill:
...
| Permission denied on scripts | Run `chmod +x ~/.hermes/skills/starlearner-nexus/scripts/*.py ~/.hermes/skills/starlearner-nexus/scripts/*.sh` |
...
## 📤 Sharing Generated Skills

Generated skills are ordinary directories on disk. There is no separate publish step — you share them the same way you share any Hermes skill:
...
4. Share the skill directory however you normally share skills:
   - **Commit it to a git repository** (public or private) so others can clone and install it
   - **Publish a git tap** that lets other Hermes agents install the skill directly
   - **Share the SKILL.md file** (and any accompanying scripts/references) with another agent

Any Hermes agent can then drop the skill into its own `~/.hermes/skills/` directory and use it immediately.

### Handling Security Scanner False Positives

When sharing skills that cover domains like cryptocurrency, finance, or development tools, expect some security scanners to flag legitimate content as suspicious. Common false positives include:
...
If a scanner flags your skill, document the legitimate context rather than trying to bypass the scan:

1. Note which specific items the scanner flagged and why it likely misread them
2. Explain why each flagged item is legitimate educational/reference content
3. Add this context as comments in the SKILL.md or in accompanying documentation so reviewers can verify it
4. If the skill is reviewed by a human maintainer, share the documented context with them for approval
